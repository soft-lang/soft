...
IF EXISTS (
    ...
    PERFORM Log(
        _NodeID   := _NodeID,
        _Severity := 'DEBUG3',
        _Message  := format('Enter %s and execute function %I.%I', Colorize(Node(_NodeID, _Short := TRUE), 'GREEN'), Colorize(_Phase, 'CYAN'), Colorize(_Function, 'MAGENTA'))
    );
...
ELSE
    PERFORM Log(
        _NodeID   := _NodeID,
        _Severity := 'DEBUG5',
        _Message  := format('Enter %s', Colorize(Node(_NodeID, _Short := TRUE), 'GREEN'))
    );
    RETURN NULL;
END IF;
